...
open class KOAlertController : UIViewController
{
    ...
    // UITextField Array
    private var _textFieldArray: Array<UITextView> = []
    ...
    public func addTextField(_ textField: @escaping (UITextView) -> Void) {
        _opsArray.append(BlockOperation(block: {
            if let tempTextField = self.alertTextField as? UITextView {
                ...
                let textF = UITextView(frame: CGRect(x: 0, y: 0, width: 0, height: 50))
                textF.translatesAutoresizingMaskIntoConstraints = false
                textF.delegate = self
                self._textFieldArray.append(textF)
                textField(textF)
    ...
    @objc private func action(_ sender: UIButton) {
        ...
        for (index, button) in _buttonArray.enumerated() {
            if (button == sender) {
                let reportStr1 = UserDefaults.standard.value(forKey: "REPORT_TEXT")
                //print(reportStr1 as Any)
                if (reportStr1 != nil) {
                    let reportStr = String(format: "%@", reportStr1 as! CVarArg)
                    let reportText = String(reportStr.filter { !" \n\t\r".contains($0) })
                    if (reportText != "") {
                        view.endEditing(true)
                        dismis(index)
                    }
...
extension KOAlertController: UITextViewDelegate
{
    public func textField(_ textField: UITextView, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool
    {
    ...
    public func textViewDidEndEditing(_ textView: UITextView)
    {
        if (textView.text.isEmpty) {
            textView.text = CommonMessage.ReportMessage()
    ...
    public func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool
    {
        ...
        if updatedText.isEmpty
        {
            ...
            UserDefaults.standard.set(textView.text, forKey: "REPORT_TEXT")
...
{
    func createCopy() -> UITextView
    {
        let archivedData = NSKeyedArchiver.archivedData(withRootObject: self)
        return NSKeyedUnarchiver.unarchiveObject(with: archivedData) as! UITextView
    }
}
