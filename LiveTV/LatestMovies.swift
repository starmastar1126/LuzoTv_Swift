...
class LatestMovies: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var LatestMoviesArray = NSMutableArray()
    ...
    func getLatestMoviesData(_ requesturl: String?)
    {
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Latest Movies Responce Data : \(responseObject)")
                self.LatestMoviesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.LatestMoviesArray.add(storeDict as Any)
                    }
                }
                print("LatestMoviesArray Count = \(self.LatestMoviesArray.count)")
                
                DispatchQueue.main.async {
                    if (self.LatestMoviesArray.count == 0) {
                        self.myCollectionView?.isHidden = true
    ...
    {
        return self.LatestMoviesArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.LatestMoviesArray.value(forKey: "movie_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        let language_name = ((self.LatestMoviesArray.value(forKey: "language_name") as! NSArray).object(at: indexPath.row) as? String)?.uppercased()
        let aStr = String(format: "   %@   ", language_name!)
        cell.lblLanguageName?.text = aStr
        
        let language_background = (self.LatestMoviesArray.value(forKey: "language_background") as! NSArray).object(at: indexPath.row) as? String
        cell.lblLanguageName?.backgroundColor = UIColor(hexString: language_background)
        
        cell.lblMovieName?.text = (self.LatestMoviesArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.LatestMoviesArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreLatestMovies()
            }
        }
    ...
    {
        let movieID = (self.LatestMoviesArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieID, forKey: "MOVIE_ID")
        let movieNAME = (self.LatestMoviesArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieNAME, forKey: "MOVIE_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailMovieViewController()
    }
    
    //===========Get Load More Latest Movie Data==========//
    func getLoadMoreLatestMovies()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreLatestMoviesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreLatestMoviesData(_ requesturl: String?)
    {
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.LatestMovies()
