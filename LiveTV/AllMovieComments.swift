...
class AllMovieComments: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextViewDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        //======Get All Movie Comments Data======//
        self.myTableView?.isHidden = true
        self.getAllMovieComments()
    }
    
    //===========Get All Movie Comments Data==========//
    func getAllMovieComments()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getAllMovieCommentsData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getAllMovieCommentsData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_user_comment", "post_id":movieID, "type":"movie"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("All Movie Comments API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("All Movie Comments Responce Data : \(responseObject)")
    ...
    {
        let postID : String = UserDefaults.standard.string(forKey: "MOVIE_ID")!
