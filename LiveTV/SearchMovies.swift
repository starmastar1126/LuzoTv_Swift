...
class SearchMovies: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var SearchMoviesArray = NSMutableArray()
    ...
    override func viewDidLoad()
    {
        ...
        //======Get Search Movies Data======//
        self.myCollectionView?.isHidden = true
        self.getSearchMovies()
    }
    
    //===========Get Search Movie Data==========//
    func getSearchMovies()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getSearchMoviesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getSearchMoviesData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_search_movies", "search_text":searchTEXT, "page":pageNo]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Search Movies API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Search Movies Responce Data : \(responseObject)")
                self.SearchMoviesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.SearchMoviesArray.add(storeDict as Any)
                    }
                }
    ...
    {
        return self.SearchMoviesArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.SearchMoviesArray.value(forKey: "movie_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        let language_name = ((self.SearchMoviesArray.value(forKey: "language_name") as! NSArray).object(at: indexPath.row) as? String)?.uppercased()
        let aStr = String(format: "   %@   ", language_name!)
        cell.lblLanguageName?.text = aStr
        
        let language_background = (self.SearchMoviesArray.value(forKey: "language_background") as! NSArray).object(at: indexPath.row) as? String
        cell.lblLanguageName?.backgroundColor = UIColor(hexString: language_background)
        
        cell.lblMovieName?.text = (self.SearchMoviesArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.SearchMoviesArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreSearchMovies()
            }
        }
    ...
    {
        let movieID = (self.SearchMoviesArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieID, forKey: "MOVIE_ID")
        let movieNAME = (self.SearchMoviesArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieNAME, forKey: "MOVIE_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailMovieViewController()
    }
    
    //===========Get Load More Search Movies Data==========//
    func getLoadMoreSearchMovies()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreSearchMoviesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreSearchMoviesData(_ requesturl: String?)
    {
        let searchTEXT : String = UserDefaults.standard.string(forKey: "SEARCH_TEXT")!
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        ...
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Load More Search Movies API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More Search Movies Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.MoviesSearch()
    }
