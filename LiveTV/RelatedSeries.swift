...
class RelatedSeries: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var RelatedSeriesArray = NSMutableArray()
    ...
    override func viewDidLoad()
    {
        ...
        //======Get Related Series Data======//
        self.myCollectionView?.isHidden = true
        self.getRelatedSeries()
    }
    
    //===========Get Related Series Data==========//
    func getRelatedSeries()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getRelatedSeriesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getRelatedSeriesData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_related_post", "post_id":seriesID, "type":"series", "cat_id":languageID, "page":pageNo]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Related Series API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Related Series Responce Data : \(responseObject)")
                self.RelatedSeriesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.RelatedSeriesArray.add(storeDict as Any)
                    }
                }
                print("RelatedSeriesArray Count = \(self.RelatedSeriesArray.count)")
                
                DispatchQueue.main.async {
                    if (self.RelatedSeriesArray.count == 0) {
                        self.myCollectionView?.isHidden = true
    ...
    {
        return self.RelatedSeriesArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.RelatedSeriesArray.value(forKey: "series_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        cell.lblSeriesName?.text = (self.RelatedSeriesArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.RelatedSeriesArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreRelatedSeries()
            }
        }
    ...
    {
        let seriesID = (self.RelatedSeriesArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(seriesID, forKey: "SERIES_ID")
        let seriesNAME = (self.RelatedSeriesArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
    ...
    //===========Get Load More Related Series Data==========//
    func getLoadMoreRelatedSeries()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreRelatedSeriesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreRelatedSeriesData(_ requesturl: String?)
    {
        let seriesID : String = UserDefaults.standard.string(forKey: "SERIES_ID")!
        let languageID : String = UserDefaults.standard.string(forKey: "LANGUAGE_ID")!
        let pageNo:String = String(self.pageNo)
        ...
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Load More Related Series API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More Related Series Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.RelatedSeries()
    }
