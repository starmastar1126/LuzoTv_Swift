...
class LanguageMovieList: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var LanguageMovieListArray = NSMutableArray()
    ...
    override func viewDidLoad()
    {
        ...
        //======Get Language Movies List Data======//
        self.myCollectionView?.isHidden = true
        self.getLanguageMoviesList()
    }
    ...
    func getLanguageMoviesList()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getLanguageMoviesListData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getLanguageMoviesListData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_movies_by_lang_id", "lang_id":languageID, "page":pageNo]
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Language Movies List Responce Data : \(responseObject)")
                self.LanguageMovieListArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.LanguageMovieListArray.add(storeDict as Any)
                    }
                }
                print("LanguageMovieListArray Count = \(self.LanguageMovieListArray.count)")
                
                DispatchQueue.main.async {
                    if (self.LanguageMovieListArray.count == 0) {
                        self.myCollectionView?.isHidden = true
    ...
    {
        return self.LanguageMovieListArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.LanguageMovieListArray.value(forKey: "movie_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        let language_name = ((self.LanguageMovieListArray.value(forKey: "language_name") as! NSArray).object(at: indexPath.row) as? String)?.uppercased()
        let aStr = String(format: "   %@   ", language_name!)
        cell.lblLanguageName?.text = aStr
        
        let language_background = (self.LanguageMovieListArray.value(forKey: "language_background") as! NSArray).object(at: indexPath.row) as? String
        cell.lblLanguageName?.backgroundColor = UIColor(hexString: language_background)
        
        cell.lblMovieName?.text = (self.LanguageMovieListArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.LanguageMovieListArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreLanguageMoviesList()
            }
        }
    ...
    {
        let movieID = (self.LanguageMovieListArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieID, forKey: "MOVIE_ID")
        let movieNAME = (self.LanguageMovieListArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieNAME, forKey: "MOVIE_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailMovieViewController()
    }
    
    //===========Get Load More Language Movies List Data==========//
    func getLoadMoreLanguageMoviesList()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreLanguageMoviesListData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreLanguageMoviesListData(_ requesturl: String?)
    {
        let languageID : String = UserDefaults.standard.string(forKey: "LANGUAGE_ID")!
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        ...
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Language Movies List API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        let languageNAME : String = UserDefaults.standard.string(forKey: "LANGUAGE_NAME")!
        self.lblheadername?.text = languageNAME
...
//        }
    ...
    //1.Load More Language Movies List
