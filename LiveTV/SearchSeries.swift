...
class SearchSeries: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var SearchSeriesArray = NSMutableArray()
    ...
    override func viewDidLoad()
    {
        ...
        //======Get Search Series Data======//
        self.myCollectionView?.isHidden = true
        self.getSearchSeries()
    }

    //===========Get Search Series Data==========//
    func getSearchSeries()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getSearchSeriesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getSearchSeriesData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_search_series", "search_text":searchTEXT, "page":pageNo]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Search Series API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Search Series Responce Data : \(responseObject)")
                self.SearchSeriesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.SearchSeriesArray.add(storeDict as Any)
                    }
                }
    ...
    {
        return self.SearchSeriesArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.SearchSeriesArray.value(forKey: "series_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        cell.lblSeriesName?.text = (self.SearchSeriesArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.SearchSeriesArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreSearchSeries()
            }
        }
    ...
    {
        let seriesID = (self.SearchSeriesArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(seriesID, forKey: "SERIES_ID")
        let seriesNAME = (self.SearchSeriesArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(seriesNAME, forKey: "SERIES_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailSeriesViewController()
    }
    
    //===========Get Load More Search Series Data==========//
    func getLoadMoreSearchSeries()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreSearchSeriesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreSearchSeriesData(_ requesturl: String?)
    {
        let searchTEXT : String = UserDefaults.standard.string(forKey: "SEARCH_TEXT")!
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More Search Series Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.SeriesSearch()
    }
