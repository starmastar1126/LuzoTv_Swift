...
class AllSeriesComments: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextViewDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        //======Get All Series Comments Data======//
        self.myTableView?.isHidden = true
        self.getAllSeriesComments()
    }
    
    //===========Get All Series Comments Data==========//
    func getAllSeriesComments()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getAllSeriesCommentsData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getAllSeriesCommentsData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_user_comment", "post_id":seriesID, "type":"series"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("All Series Comments API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("All Series Comments Responce Data : \(responseObject)")
    ...
    {
        let postID : String = UserDefaults.standard.string(forKey: "SERIES_ID")!
