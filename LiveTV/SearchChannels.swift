...
class SearchChannels: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var SearchChannelsArray = NSMutableArray()
    ...
    func getSearchChannels()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getSearchChannelsData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getSearchChannelsData(_ requesturl: String?)
    {
        ...
        let dict = ["salt":salt, "sign":sign, "method_name":"get_search_channels", "search_text":searchTEXT, "page":pageNo]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Search Channels API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Search Channels Responce Data : \(responseObject)")
                self.SearchChannelsArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.SearchChannelsArray.add(storeDict as Any)
                    }
                }
                print("SearchChannelsArray Count = \(self.SearchChannelsArray.count)")
                
                DispatchQueue.main.async {
                    if (self.SearchChannelsArray.count == 0) {
                        self.myCollectionView?.isHidden = true
    ...
    {
        return self.SearchChannelsArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.SearchChannelsArray.value(forKey: "channel_thumbnail") as! NSArray).object(at: indexPath.row) as? String
        ...
        cell.lblChannelName?.text = (self.SearchChannelsArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.SearchChannelsArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreSearchChannels()
            }
        }
    ...
    {
        let channelID = (self.SearchChannelsArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelID, forKey: "CHANNEL_ID")
        let channelNAME = (self.SearchChannelsArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelNAME, forKey: "CHANNEL_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailChannelViewController()
    }
    
    //===========Get Load More Search Channel Data==========//
    func getLoadMoreSearchChannels()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreSearchChannelsData(encodedString)
        } else {
            self.InternetConnectionNotAvailable2()
        }
    }
    func getLoadMoreSearchChannelsData(_ requesturl: String?)
    {
        let searchTEXT : String = UserDefaults.standard.string(forKey: "SEARCH_TEXT")!
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        ...
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Load More Search Channels API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More Search Channels Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.ChannelsSearch()
    }
