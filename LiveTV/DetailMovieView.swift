...
let kCastControlBarsAnimationDuration1: TimeInterval = 0.20

class DetailMovieView: UIViewController,TPFloatRatingViewDelegate,UIWebViewDelegate, UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITableViewDelegate,UITableViewDataSource,GCKSessionManagerListener,GCKRemoteMediaClientListener,GCKUIMiniMediaControlsViewControllerDelegate,GCKRequestDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    func updateControlBarsVisibility()
    {
        ...
        UIView.animate(withDuration: kCastControlBarsAnimationDuration1, animations: { () -> Void in
    ...
    {
        let videoType = (self.DetailMovieArray.value(forKey: "video_type") as! NSArray).componentsJoined(by: "")
        if (videoType == "youtube_url") {
            ...
            let movie_title = (self.DetailMovieArray.value(forKey: "movie_title") as! NSArray).componentsJoined(by: "")
            let language_name = (self.DetailMovieArray.value(forKey: "language_name") as! NSArray).componentsJoined(by: "")
            //let movie_desc = (self.DetailMovieArray.value(forKey: "movie_desc") as! NSArray).componentsJoined(by: "")
            let movie_cover = (self.DetailMovieArray.value(forKey: "movie_cover") as! NSArray).componentsJoined(by: "")
            ...
            let movie_url = (self.DetailMovieArray.value(forKey: "movie_url") as! NSArray).componentsJoined(by: "")
    ...
    func getUserProfileData(_ requesturl: String?)
    {
        ...
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
            { task, responseObject in if let responseObject = responseObject
            {
                ...
                print("ProfileData Count = \(self.ProfileArray)")
    ...
    func getSingleMovie()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getSingleMovieData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getSingleMovieData(_ requesturl: String?)
    {
        let movieID : String = UserDefaults.standard.string(forKey: "MOVIE_ID")!
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"get_single_movie", "movie_id":movieID]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Single Movie API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
                print("Single Movie Responce Data : \(responseObject)")
                self.DetailMovieArray.removeAllObjects()
                ...
                DispatchQueue.main.async {
                    if (self.DetailMovieArray.count == 0) {
                        self.myScrollView?.isHidden = true
    ...
    func setPlayVideo()
    {
        ...
        let videoType = (self.DetailMovieArray.value(forKey: "video_type") as! NSArray).componentsJoined(by: "")
        if (videoType == "youtube_url") {
            let movieUrl = (self.DetailMovieArray.value(forKey: "movie_url") as! NSArray).componentsJoined(by: "")
            ...
            let movieStr = (self.DetailMovieArray.value(forKey: "movie_url") as! NSArray).componentsJoined(by: "")
            print("Load URL ->",movieStr)
//            let encodeString = movieStr.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)
            let movieUrl = URL(string: movieStr)
    ...
    func setDataIntoScrollView()
    {
        ...
        //2.Check Movie Favourite or Not
        let modalObj: Modal = Modal()
        modalObj.mid = (self.DetailMovieArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        let isNewsExist = Singleton.getInstance().SingleMoviesQueryData(modalObj)
        ...
        let movie_title = (self.DetailMovieArray.value(forKey: "movie_title") as! NSArray).componentsJoined(by: "")
        self.lblmoviename?.text = movie_title
        self.lblheadername?.text = movie_title

        //4.Movie Language Name
        let language_name = (self.DetailMovieArray.value(forKey: "language_name") as! NSArray).componentsJoined(by: "")
        self.lblLanguageName?.text = language_name
        
        //5.Movie Total Views
        let total_views = (self.DetailMovieArray.value(forKey: "total_views") as! NSArray).componentsJoined(by: "")
        ...
        let rate_avg = (self.DetailMovieArray.value(forKey: "rate_avg") as! NSArray).componentsJoined(by: "")
        ...
        //7.Total Rating
        let total_rate = (self.DetailMovieArray.value(forKey: "total_rate") as! NSArray).componentsJoined(by: "")
        ...
        DispatchQueue.main.async {
            let movie_desc = (self.DetailMovieArray.value(forKey: "movie_desc") as! NSArray).componentsJoined(by: "")
            let htmlData = NSString(format: "<font face='Montserrat-Medium' size='3' color='#808080'>%@", movie_desc)
    ...
    func webViewDidFinishLoad(_ webView: UIWebView)
    {
        ...
        //3.myView3
        self.myView3?.frame = CGRect(x: 0, y: self.view1Height+self.view2Height, width: UIScreen.main.bounds.width, height: 55+(CommonUtils.screenWidth/2.5))
        if (self.RelatedArray.count == 0) {
    ...
    @IBAction func OnPlayClick(sender:UIButton)
    {
        ...
        if (videoType == "embedded_url") {
            let movieStr = (self.DetailMovieArray.value(forKey: "movie_url") as! NSArray).componentsJoined(by: "")
            UserDefaults.standard.set(movieStr, forKey:"IFRAME")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.RelatedArray.value(forKey: "movie_poster") as! NSArray).object(at: indexPath.row) as? String
        ...
        let language_name = ((self.RelatedArray.value(forKey: "language_name") as! NSArray).object(at: indexPath.row) as? String)?.uppercased()
        let aStr = String(format: "   %@   ", language_name!)
        cell.lblLanguageName?.text = aStr
        
        let language_background = (self.RelatedArray.value(forKey: "language_background") as! NSArray).object(at: indexPath.row) as? String
        cell.lblLanguageName?.backgroundColor = UIColor(hexString: language_background)
        
        cell.lblMovieName?.text = (self.RelatedArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
    ...
    {
        let movieID = (self.RelatedArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(movieID, forKey: "MOVIE_ID")
        let movieNAME = (self.RelatedArray.value(forKey: "movie_title") as! NSArray).object(at: indexPath.row) as? String
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat
    {
        let adminCommentMsg : String = "It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout."
        //let adminCommentMsg : String = (self.CommentArray.value(forKey: "comment_text") as! NSArray).object(at: indexPath.row) as! String
        let lbladminComment = UILabel(frame: CGRect(x: 125, y: 25, width: tableView.frame.size.width - 135, height: 25))
        let maximumLabelSize = CGSize(width: .min, height: .max)
        var textRect: CGRect? = nil
        if let font = UIFont(name: "Poppins-Regular", size: 14.0) {
            textRect = adminCommentMsg
            .boundingRect(with: maximumLabelSize, options: .usesLineFragmentOrigin, attributes: [
                NSAttributedString.Key.font: font
                ], context: nil)
        }
        let expectedLabelSize = textRect?.size
        var newFrame = lbladminComment.frame
        newFrame.size.height = expectedLabelSize?.height ?? 0.0
        lbladminComment.frame = newFrame
        lbladminComment.numberOfLines = 0
        lbladminComment.text = adminCommentMsg
        lbladminComment.sizeToFit()
        let hieght = getLabelHeight(lbladminComment)
        return hieght + 54
    }
    ...
    {
        let postID = (self.DetailMovieArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        let userID : String = UserDefaults.standard.string(forKey: "USER_ID")!
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"my_rating", "post_id":postID, "user_id":userID, "type":"movie"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    func getNewUserRatingData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"user_rating", "post_id":postID, "user_id":userID, "rate":rating, "type":"movie"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    func sendUserReportData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"user_report", "user_id":userID, "post_id":postID, "report":reportText, "type":"movie"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    @IBAction func OnFavouriteClick(sender:UIButton)
    {
        let modalObj: Modal = Modal()
        modalObj.mid = (self.DetailMovieArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        modalObj.movie_title = (self.DetailMovieArray.value(forKey: "movie_title") as! NSArray).componentsJoined(by: "")
        modalObj.language_name = (self.DetailMovieArray.value(forKey: "language_name") as! NSArray).componentsJoined(by: "")
        modalObj.language_background = (self.DetailMovieArray.value(forKey: "language_background") as! NSArray).componentsJoined(by: "")
        modalObj.movie_cover = (self.DetailMovieArray.value(forKey: "movie_cover") as! NSArray).componentsJoined(by: "")
        modalObj.movie_cover_thumb = (self.DetailMovieArray.value(forKey: "movie_cover_thumb") as! NSArray).componentsJoined(by: "")
        modalObj.movie_poster = (self.DetailMovieArray.value(forKey: "movie_poster") as! NSArray).componentsJoined(by: "")
        modalObj.movie_poster_thumb = (self.DetailMovieArray.value(forKey: "movie_poster_thumb") as! NSArray).componentsJoined(by: "")
        let isNewsExist = Singleton.getInstance().SingleMoviesQueryData(modalObj)
        if (isNewsExist.count != 0)
        {
            let isDeleted = Singleton.getInstance().DeleteMoviesQueryData(modalObj)
            if (isDeleted) {
                self.btnFav?.setBackgroundImage(UIImage(named: "ic_fav")!, for: UIControl.State.normal)
            ...
            let isInserted = Singleton.getInstance().InsertMoviesQueryData(modalObj)
            if (isInserted) {
                self.btnFav?.setBackgroundImage(UIImage(named: "ic_fav_hov")!, for: UIControl.State.normal)
    ...
    //==========Related Movies View All Click==========//
    @IBAction func OnRelatedMoviesViewAllClick(sender:UIButton)
    {
        let language_id = (self.DetailMovieArray.value(forKey: "language_id") as! NSArray).componentsJoined(by: "")
        UserDefaults.standard.set(language_id, forKey: "LANGUAGE_ID")
        self.CallRelatedMoviesViewController()
    }
    
    //==========Comments View All Button Click==========//
    @IBAction func OnCommentsViewAllClick(sender:UIButton)
    {
        self.CallAllMovieCommentsViewController()
    }
    ...
    func setUserCommentData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign as Any, "method_name":"user_comment", "post_id":postID, "user_id":userID, "comment_text":commentText, "type":"movie", "is_limit":"true"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    func CallRelatedMoviesViewController()
    {
        let view : UIViewController
        if (UI_USER_INTERFACE_IDIOM() == .pad) {
            view = RelatedMovies(nibName: "RelatedMovies_iPad", bundle: nil)
        } else if (CommonUtils.screenHeight >= 812) {
            view = RelatedMovies(nibName: "RelatedMovies_iPhoneX", bundle: nil)
        } else {
            view = RelatedMovies(nibName: "RelatedMovies", bundle: nil)
        }
        self.navigationController?.pushViewController(view, animated: true)
    }
    ...
    func CallAllMovieCommentsViewController()
    {
        let view : UIViewController
        if (UI_USER_INTERFACE_IDIOM() == .pad) {
            view = AllMovieComments(nibName: "AllMovieComments_iPad", bundle: nil)
        } else if (CommonUtils.screenHeight >= 812) {
            view = AllMovieComments(nibName: "AllMovieComments_iPhoneX", bundle: nil)
        } else {
            view = AllMovieComments(nibName: "AllMovieComments", bundle: nil)
        }
        self.navigationController?.pushViewController(view, animated: true)
    }
    ...
    {
        let modalObj: Modal = Modal()
        modalObj.rid = (self.DetailMovieArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        modalObj.title = (self.DetailMovieArray.value(forKey: "movie_title") as! NSArray).componentsJoined(by: "")
        modalObj.cover_image = (self.DetailMovieArray.value(forKey: "movie_poster") as! NSArray).componentsJoined(by: "")
        modalObj.type = "movie"
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        //let movieNAME : String = UserDefaults.standard.string(forKey: "MOVIE_NAME")!
        //self.lblheadername?.text = movieNAME
        
        //5.Related Movies
        self.lblRelatedMovies?.text = CommonMessage.RelatedMovies()
        
        //6.Related Movies View All
        self.btnRelatedViewAll?.setTitle(CommonMessage.ViewAll(), for: .normal)
        
        //7.No Related Movies Found
        self.lblnoRelatedMovieFound?.text = CommonMessage.NoRelatedMoviesFound()
