...
let kCastControlBarsAnimationDuration3: TimeInterval = 0.20

class DetailSeriesView: UIViewController,TPFloatRatingViewDelegate,UIWebViewDelegate, UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITableViewDelegate,UITableViewDataSource,GCKSessionManagerListener,GCKRemoteMediaClientListener,GCKUIMiniMediaControlsViewControllerDelegate,GCKRequestDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var DetailSeriesArray = NSMutableArray()
    ...
    @IBOutlet weak var lblseriesname: MarqueeLabel!
    @IBOutlet var lblTotalNoOfSeason : UILabel?
    ...
    @IBOutlet var btnSeason : UIButton?
    ...
    @IBOutlet var lblRelatedSeries : UILabel?
    @IBOutlet var btnRelatedViewAll : UIButton?
    @IBOutlet var myCollectionView : UICollectionView?
    @IBOutlet var lblnoRelatedSeriesFound : UILabel?
    @IBOutlet var lblnoSeasonEpisodeFound : UILabel?
    var SeasonsArray = NSArray()
    var EpisodesArray = NSMutableArray()
    ...
    func updateControlBarsVisibility()
    {
        ...
        UIView.animate(withDuration: kCastControlBarsAnimationDuration3, animations: { () -> Void in
    ...
    {
        let videoType = (self.EpisodesArray.value(forKey: "episode_type") as! NSArray).object(at: self.episodeINDEX) as? String
        if (videoType == "youtube_url") {
            ...
            let movie_title = (self.EpisodesArray.value(forKey: "episode_title") as! NSArray).object(at: self.episodeINDEX) as? String
            let episode_poster = (self.EpisodesArray.value(forKey: "episode_poster") as! NSArray).object(at: self.episodeINDEX) as? String
            let encodeString = episode_poster?.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)
            ...
            let episode_url = (self.DetailSeriesArray.value(forKey: "episode_url") as! NSArray).object(at: self.episodeINDEX) as? String
            let url = URL(string: episode_url!)
            ...
            let movie_title = (self.EpisodesArray.value(forKey: "episode_title") as! NSArray).object(at: self.episodeINDEX) as? String
            let episode_poster = (self.EpisodesArray.value(forKey: "episode_poster") as! NSArray).componentsJoined(by: "")
            let encodeString = episode_poster.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)
            let movieImage = URL(string: encodeString!)
            ...
            let episode_url = (self.EpisodesArray.value(forKey: "episode_url") as! NSArray).object(at: self.episodeINDEX) as? String
            let url = URL(string: episode_url!)
            let mediaInfoBuilder = GCKMediaInformationBuilder(contentURL: url!)
            ...
            let episode_poster = (self.EpisodesArray.value(forKey: "episode_poster") as! NSArray).object(at: self.episodeINDEX) as? String
            let encodedString = episode_poster?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
    ...
    override func viewDidLoad()
    {
        ...
        if (UI_USER_INTERFACE_IDIOM() == .pad) {
            let nibName = UINib(nibName: "EpisodeCell_iPad", bundle:nil)
            self.myCollectionView2?.register(nibName, forCellWithReuseIdentifier: "cell")
        } else {
            let nibName = UINib(nibName: "EpisodeCell", bundle:nil)
            self.myCollectionView2?.register(nibName, forCellWithReuseIdentifier: "cell")
        }
        ...
        self.getSingleSeries()
    }
    
    //===========Get Single Series Data==========//
    func getSingleSeries()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            ...
            self.getSingleSeriesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable1()
        }
    }
    func getSingleSeriesData(_ requesturl: String?)
    {
        let seriesID : String = UserDefaults.standard.string(forKey: "SERIES_ID")!
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"get_single_series", "series_id":seriesID]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Single Series API URL : \(strDict)")
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Single Series Responce Data : \(responseObject)")
                self.DetailSeriesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    self.DetailSeriesArray.add(storeDict)
                    self.SeasonsArray = storeDict["seasons"] as! NSArray
                    self.RelatedArray = storeDict["related"] as! NSArray
                    self.CommentArray = storeDict["comments"] as! NSArray
                }
                print("DetailSeriesArray Count = \(self.DetailSeriesArray.count)")
                print("SeasonsArray Count = \(self.SeasonsArray.count)")
                ...
                DispatchQueue.main.async {
                    if (self.SeasonsArray.count == 0) {
                        self.myView1?.frame = CGRect(x: 10, y: 15, width: CommonUtils.screenWidth-20, height: 115)
                        self.myView5?.isHidden = true
                        self.view5Height = 0
                        ...
                        self.myView1?.frame = CGRect(x: 10, y: 15, width: CommonUtils.screenWidth-20, height: 165)
                        self.myView5?.isHidden = false
                        self.view5Height = 165
                        self.seasonView?.isHidden = false
                        self.getSeriesSeasonEpisodes()
                    }
                }
                ...
                DispatchQueue.main.async {
                    if (self.RelatedArray.count == 0) {
                        ...
                        self.lblnoRelatedSeriesFound?.isHidden = false
                    } else {
                        self.myCollectionView?.isHidden = false
                        self.lblnoRelatedSeriesFound?.isHidden = true
                ...
                DispatchQueue.main.async {
                    if (self.DetailSeriesArray.count == 0) {
                        self.myScrollView?.isHidden = true
    ...
    //===========Get Series Season Episodes Data==========//
    func getSeriesSeasonEpisodes()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getSeriesSeasonEpisodesData(encodedString)
        } else {
            self.InternetConnectionNotAvailable6()
        }
    }
    func getSeriesSeasonEpisodesData(_ requesturl: String?)
    {
        if (self.seriesID == nil) {
            self.seriesID = (self.DetailSeriesArray.value(forKey: "id") as! NSArray).object(at: 0) as? String
            print("Series ID = \(String(describing: self.seriesID))")
        }
        
        if (self.seasonID == nil) {
            self.seasonID = (self.SeasonsArray.value(forKey: "id") as! NSArray).object(at: 0) as? String
            print("Season ID = \(String(describing: self.seasonID))")
        }
        
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"get_episodes", "series_id":self.seriesID, "season_id":self.seasonID]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
        print("Series Season Episodes API URL : \(strDict)")
        let manager = AFHTTPSessionManager()
        manager.post(requesturl!, parameters: strDict, progress: nil, success:
        { task, responseObject in if let responseObject = responseObject
            {
                print("Series Season Episodes Responce Data : \(responseObject)")
                self.EpisodesArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.EpisodesArray.add(storeDict as Any)
                    }
                }
                print("EpisodesArray Count = \(self.EpisodesArray.count)")
                
                DispatchQueue.main.async {
                    if (self.EpisodesArray.count == 0) {
                        self.myCollectionView2?.isHidden = true
                        self.lblnoSeasonEpisodeFound?.isHidden = false
                    } else {
                        self.myCollectionView2?.isHidden = false
                        self.lblnoSeasonEpisodeFound?.isHidden = true
    ...
    func setPlayVideo()
    {
        ...
        if (videoType == "youtube_url") {
            ...
            let video_id = CommonUtils.extractYoutubeId(fromLink: SeriesUrl)
            print(video_id ?? "")
            ...
            let SeriesStr = (self.EpisodesArray.value(forKey: "episode_url") as! NSArray).object(at: self.episodeINDEX) as! String
//            let encodeString = SeriesStr.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)
            let SeriesUrl = URL(string: SeriesStr)
            print("SeriesUrl ->",SeriesUrl)
            let player = AVPlayer(url: SeriesUrl!)
...
//            player.isClosedCaptionDisplayEnabled = false
            ...
            let encodeString = seriesStr.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)
            let seriesUrl = URL(string: encodeString!)
            let player = AVPlayer(url: seriesUrl!)
            let controller = AVPlayerViewController()
            self.addChild(controller)
            self.playerView?.addSubview(controller.view)
...
//            controller.player?.playImmediately(atRate: 1.0)
    ...
    func setDataIntoScrollView()
    {
        ...
        //2.Check Series Favourite or Not
        let modalObj: Modal = Modal()
        modalObj.mid = (self.DetailSeriesArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        let isNewsExist = Singleton.getInstance().SingleSeriesQueryData(modalObj)
        ...
        //3.Series Name
        self.lblseriesname.type = .continuous
        self.lblseriesname.animationCurve = .easeInOut
        let series_name = (self.DetailSeriesArray.value(forKey: "series_name") as! NSArray).componentsJoined(by: "")
        self.lblseriesname?.text = series_name
        self.lblheadername?.text = series_name
        
        //4.Series Season Name
        let totalNoOfSeason = String(format: "Total No Of Season - %@",String(self.SeasonsArray.count))
        print(totalNoOfSeason)
        self.lblTotalNoOfSeason?.text = totalNoOfSeason
        
        //5.Series Total Views
        let total_views = (self.DetailSeriesArray.value(forKey: "total_views") as! NSArray).componentsJoined(by: "")
        ...
        let rate_avg = (self.DetailSeriesArray.value(forKey: "rate_avg") as! NSArray).componentsJoined(by: "")
        ...
        //7.Total Rating
        let total_rate = (self.DetailSeriesArray.value(forKey: "total_rate") as! NSArray).componentsJoined(by: "")
        ...
        //9.Series Name
        let season_name = (self.SeasonsArray.value(forKey: "season_name") as! NSArray).object(at: 0)
        self.lblSeason?.text = season_name as? String
        self.lblSeasonName?.text = season_name as? String
        
        //10.Description
        DispatchQueue.main.async {
            let series_desc = (self.DetailSeriesArray.value(forKey: "series_desc") as! NSArray).componentsJoined(by: "")
            let htmlData = NSString(format: "<font face='Montserrat-Medium' size='3' color='#808080'>%@", series_desc)
    ...
    func webViewDidFinishLoad(_ webView: UIWebView)
    {
        ...
        if (self.SeasonsArray.count == 0) {
            ...
            self.view1Height = 180
        }
        
        //2.myView2
        self.myView2?.frame = CGRect(x: 0, y: self.view5Height+self.view1Height, width: UIScreen.main.bounds.width, height: 50+web1Height!)
        let series_desc = (self.DetailSeriesArray.value(forKey: "series_desc") as! NSArray).componentsJoined(by: "")
        if (series_desc == "") {
        ...
        //3.myView3
        self.myView3?.frame = CGRect(x: 0, y: self.view5Height+self.view1Height+self.view2Height, width: UIScreen.main.bounds.width, height: 55+(CommonUtils.screenWidth/2.5))
        if (self.RelatedArray.count == 0) {
        ...
        self.myView4?.frame = CGRect(x: 0, y: self.view5Height+self.view1Height+self.view2Height+self.view3Height, width: UIScreen.main.bounds.width, height: 65+tblHeight+70)
        ...
        if (isBannerAd == "true") {
            self.myScrollView?.contentSize = CGSize(width: UIScreen.main.bounds.width, height: (self.view5Height+self.view1Height+self.view2Height+self.view3Height+self.view4Height+50))
        } else {
            self.myScrollView?.contentSize = CGSize(width: UIScreen.main.bounds.width, height: (self.view5Height+self.view1Height+self.view2Height+self.view3Height+self.view4Height))
        }
    }
    ...
    @IBAction func OnPlayClick(sender:UIButton)
    {
        ...
        if (videoType == "embedded_url") {
            let seriesStr = (self.DetailSeriesArray.value(forKey: "episode_url") as! NSArray).componentsJoined(by: "")
            UserDefaults.standard.set(seriesStr, forKey:"IFRAME")
    ...
    {
        if (collectionView == self.myCollectionView2) {
    ...
    {
        if (collectionView == self.myCollectionView2) {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath as IndexPath) as! EpisodeCell
            
            let strimgpath : String? = (self.EpisodesArray.value(forKey: "episode_poster") as! NSArray).object(at: indexPath.row) as? String
            ...
            cell.lblEpisodeName?.text = (self.EpisodesArray.value(forKey: "episode_title") as! NSArray).object(at: indexPath.row) as? String
            
            if (indexPath.row == self.episodeINDEX) {
                cell.lblPlayingNow?.isHidden = false
            } else {
                cell.lblPlayingNow?.isHidden = true
            }
            ...
            let strimgpath : String? = (self.RelatedArray.value(forKey: "series_poster") as! NSArray).object(at: indexPath.row) as? String
            let encodedString = strimgpath?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            let url = URL(string: encodedString!)
            let placeImage = UIImage(named: "placeholder_small")
            cell.iconImageView?.sd_setImage(with: url, placeholderImage: placeImage, options: .continueInBackground, completed: nil)
            
            cell.lblSeriesName?.text = (self.RelatedArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        if (collectionView == self.myCollectionView2) {
            ...
            let seriesID = (self.RelatedArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
            UserDefaults.standard.set(seriesID, forKey: "SERIES_ID")
            let seriesNAME = (self.RelatedArray.value(forKey: "series_name") as! NSArray).object(at: indexPath.row) as? String
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        /*var cell = tableView.dequeueReusableCell(withIdentifier: "cell")
         ...
         //1.User Icon
         let userIcon = UIImageView(frame: CGRect(x: 7, y: 7, width: 60, height: 60))
         userIcon.layer.cornerRadius = userIcon.frame.size.height / 2
         userIcon.clipsToBounds = true
         userIcon.image = UIImage(named: "logo_comment")
         cell?.contentView.addSubview(userIcon)
         
         //2.User Name
         let lblUserName = UILabel(frame: CGRect(x: 75, y: 5, width: tableView.frame.size.width - 85, height: 40))
         lblUserName.backgroundColor = UIColor.gray
         lblUserName.textColor = UIColor(hexString: "#FFFFFF", alpha: 0.7)
         lblUserName.font = UIFont(name: "Poppins-Medium", size: 17.0)
         let userName : String = (self.CommentArray.value(forKey: "user_name") as! NSArray).object(at: indexPath.row) as! String
         lblUserName.text = userName
         cell?.contentView.addSubview(lblUserName)
         
         //3.User Comment
         let userCommentMsg : String = "It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout."
         
         //let userCommentMsg : String = (self.CommentArray.value(forKey: "comment_text") as! NSArray).object(at: indexPath.row) as! String
         let lblUserComment = UILabel(frame: CGRect(x: 75, y: 45, width: tableView.frame.size.width - 85, height: 35))
         let maximumLabelSize = CGSize(width: .min, height: .max)
         var textRect: CGRect? = nil
         if let font = UIFont(name: "Poppins-Regular", size: 14.0) {
         textRect = userCommentMsg.boundingRect(with: maximumLabelSize, options: .usesLineFragmentOrigin, attributes: [
         NSAttributedString.Key.font: font
         ], context: nil)
         }
         let expectedLabelSize = textRect?.size
         var newFrame = lblUserComment.frame
         newFrame.size.height = expectedLabelSize?.height ?? 0.0
         lblUserComment.frame = newFrame
         lblUserComment.numberOfLines = 0
         lblUserComment.text = userCommentMsg
         lblUserComment.sizeToFit()
         lblUserComment.backgroundColor = UIColor.green
         lblUserComment.textColor = UIColor(hexString: "#FFFFFF", alpha: 0.5)
         cell?.contentView.addSubview(lblUserComment)
    ...
    {
        let postID = (self.DetailSeriesArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        let userID : String = UserDefaults.standard.string(forKey: "USER_ID")!
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"my_rating", "post_id":postID, "user_id":userID, "type":"series"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    func getNewUserRatingData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"user_rating", "post_id":postID, "user_id":userID, "rate":rating, "type":"series"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    func sendUserReportData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign, "method_name":"user_report", "user_id":userID, "post_id":postID, "report":reportText, "type":"series"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    {
        let modalObj: Modal = Modal()
        modalObj.sid = (self.DetailSeriesArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        modalObj.series_name = (self.DetailSeriesArray.value(forKey: "series_name") as! NSArray).componentsJoined(by: "")
        modalObj.series_cover = (self.DetailSeriesArray.value(forKey: "series_cover") as! NSArray).componentsJoined(by: "")
        modalObj.series_cover_thumb = (self.DetailSeriesArray.value(forKey: "series_cover_thumb") as! NSArray).componentsJoined(by: "")
        modalObj.series_poster = (self.DetailSeriesArray.value(forKey: "series_poster") as! NSArray).componentsJoined(by: "")
        modalObj.series_poster_thumb = (self.DetailSeriesArray.value(forKey: "series_poster_thumb") as! NSArray).componentsJoined(by: "")
        let isNewsExist = Singleton.getInstance().SingleSeriesQueryData(modalObj)
        if (isNewsExist.count != 0)
        {
            let isDeleted = Singleton.getInstance().DeleteSeriesQueryData(modalObj)
            if (isDeleted) {
                self.btnFav?.setBackgroundImage(UIImage(named: "ic_fav")!, for: UIControl.State.normal)
            ...
            let isInserted = Singleton.getInstance().InsertSeriesQueryData(modalObj)
            if (isInserted) {
                self.btnFav?.setBackgroundImage(UIImage(named: "ic_fav_hov")!, for: UIControl.State.normal)
    ...
    //==========Related Series View All Click==========//
    @IBAction func OnRelatedSeriesViewAllClick(sender:UIButton)
    {
        let language_id = (self.DetailSeriesArray.value(forKey: "language_id") as! NSArray).componentsJoined(by: "")
        UserDefaults.standard.set(language_id, forKey: "LANGUAGE_ID")
        self.CallRelatedSeriesViewController()
    }
    
    //==========Comments View All Button Click==========//
    @IBAction func OnCommentsViewAllClick(sender:UIButton)
    {
        self.CallAllSeriesCommentsViewController()
    }
    ...
    func setUserCommentData(_ requesturl: String?)
    {
        ...
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        let dict = ["salt":salt, "sign":sign as Any, "method_name":"user_comment", "post_id":postID, "user_id":userID, "comment_text":commentText, "type":"series", "is_limit":"true"]
        let data = CommonUtils.getBase64EncodedString(dict as [AnyHashable : Any])
        let strDict = ["data": data]
    ...
    //==========Season Button Click==========//
    @IBAction func OnSeasonClick(sender:UIButton)
    {
        let series_name = (self.DetailSeriesArray.value(forKey: "series_name") as! NSArray).componentsJoined(by: "")
        SRPopview.show(withValues: self.SeasonsArray.value(forKey: "season_name") as! Array<String>, heading: series_name) { (result) in
            switch result {
            case .notPicked:
            ...
            case .picked(let str, let index):
                ...
                self.lblSeason?.text = str
                self.lblSeasonName?.text = str
                
                self.episodeINDEX = 0
                self.seasonID = (self.SeasonsArray.value(forKey: "id") as! NSArray).object(at: index) as? String
                self.getSeriesSeasonEpisodes()
            }
        }
    }
    ...
    func CallRelatedSeriesViewController()
    {
        let view : UIViewController
        if (UI_USER_INTERFACE_IDIOM() == .pad) {
            view = RelatedSeries(nibName: "RelatedSeries_iPad", bundle: nil)
        } else if (CommonUtils.screenHeight >= 812) {
            view = RelatedSeries(nibName: "RelatedSeries_iPhoneX", bundle: nil)
        } else {
            view = RelatedSeries(nibName: "RelatedSeries", bundle: nil)
        }
        self.navigationController?.pushViewController(view, animated: true)
    }
    ...
    func CallAllSeriesCommentsViewController()
    {
        let view : UIViewController
        if (UI_USER_INTERFACE_IDIOM() == .pad) {
            view = AllSeriesComments(nibName: "AllSeriesComments_iPad", bundle: nil)
        } else if (CommonUtils.screenHeight >= 812) {
            view = AllSeriesComments(nibName: "AllSeriesComments_iPhoneX", bundle: nil)
        } else {
            view = AllSeriesComments(nibName: "AllSeriesComments", bundle: nil)
        }
        self.navigationController?.pushViewController(view, animated: true)
    }
    ...
    {
        let modalObj: Modal = Modal()
        modalObj.rid = (self.DetailSeriesArray.value(forKey: "id") as! NSArray).componentsJoined(by: "")
        modalObj.title = (self.DetailSeriesArray.value(forKey: "series_name") as! NSArray).componentsJoined(by: "")
        modalObj.cover_image = (self.DetailSeriesArray.value(forKey: "series_poster") as! NSArray).componentsJoined(by: "")
        modalObj.type = "series"
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        //let seriesNAME : String = UserDefaults.standard.string(forKey: "SERIES_NAME")!
        //self.lblheadername?.text = seriesNAME
        
        //5.Related Series
        self.lblRelatedSeries?.text = CommonMessage.RelatedSeries()
        
        //6.Related Series View All
        self.btnRelatedViewAll?.setTitle(CommonMessage.ViewAll(), for: .normal)
        
        //7.No Related Series Found
        self.lblnoRelatedSeriesFound?.text = CommonMessage.NoRelatedSeriesFound()
        ...
        //15.Season View
        self.seasonView?.layer.cornerRadius = (self.seasonView?.frame.size.height)!/2
        self.seasonView?.clipsToBounds = true
        
        //16.Season Button
        self.btnSeason?.layer.cornerRadius = (self.btnSeason?.frame.size.height)!/2
        self.btnSeason?.clipsToBounds = true
    }
    ...
    //6.Series Season Episodes
    func InternetConnectionNotAvailable6()
    {
        let alert = SCLAlertView()
        _ = alert.addButton(CommonMessage.RETRY()) {
            self.getSeriesSeasonEpisodes()
