...
class CategoryList: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        //======Get TV Categories List Data======//
        self.myCollectionView?.isHidden = true
        self.getTVCategoriesList()
    }
    ...
    func getTVCategoriesData(_ requesturl: String?)
    {
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("TV Categories List Responce Data : \(responseObject)")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.CatListArray.value(forKey: "channel_thumbnail") as! NSArray).object(at: indexPath.row) as? String
        ...
        cell.lblChannelName?.text = (self.CatListArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.CatListArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreTVCategoriesList()
            }
        }
    ...
    {
        let channelID = (self.CatListArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelID, forKey: "CHANNEL_ID")
        let channelNAME = (self.CatListArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelNAME, forKey: "CHANNEL_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailChannelViewController()
    }
    
    //===========Get Load More TV Categories List Data==========//
    func getLoadMoreTVCategoriesList()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
    ...
    func getLoadMoreTVCategoriesData(_ requesturl: String?)
    {
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More TV Categories List Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        let catNAME : String = UserDefaults.standard.string(forKey: "CAT_NAME")!
