...
class LatestChannel: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,GADBannerViewDelegate,GADInterstitialDelegate
{
    ...
    var LatestChannelArray = NSMutableArray()
    ...
    func getLatestChannelData(_ requesturl: String?)
    {
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Latest Channel Responce Data : \(responseObject)")
                self.LatestChannelArray.removeAllObjects()
                ...
                for i in 0..<storeArr.count {
                    ...
                    if storeDict != nil {
                        self.LatestChannelArray.add(storeDict as Any)
                    }
                }
                print("LatestChannelArray Count = \(self.LatestChannelArray.count)")
                
                DispatchQueue.main.async {
                    if (self.LatestChannelArray.count == 0) {
                        self.myCollectionView?.isHidden = true
    ...
    {
        return self.LatestChannelArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        let strimgpath : String? = (self.LatestChannelArray.value(forKey: "channel_thumbnail") as! NSArray).object(at: indexPath.row) as? String
        ...
        cell.lblChannelName?.text = (self.LatestChannelArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        
        //Lazy loading
        if (isLoadMore) {
            if (indexPath.row == self.LatestChannelArray.count-1) {
                self.pageNo = self.pageNo + 1;
                self.getLoadMoreLatestChannel()
            }
        }
    ...
    {
        let channelID = (self.LatestChannelArray.value(forKey: "id") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelID, forKey: "CHANNEL_ID")
        let channelNAME = (self.LatestChannelArray.value(forKey: "channel_title") as! NSArray).object(at: indexPath.row) as? String
        UserDefaults.standard.set(channelNAME, forKey: "CHANNEL_NAME")
        self.CallAdmobInterstitial()
        self.CallDetailChannelViewController()
    }
    
    //===========Get Load More Latest Channel Data==========//
    func getLoadMoreLatestChannel()
    {
        if (Reachability.shared.isConnectedToNetwork()) {
            self.startSpinner()
            let str = String(format: "%@api.php",CommonUtils.getBaseUrl())
            let encodedString = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            self.getLoadMoreLatestChannelData(encodedString)
        } else {
            self.InternetConnectionNotAvailable()
        }
    }
    func getLoadMoreLatestChannelData(_ requesturl: String?)
    {
        let pageNo:String = String(self.pageNo)
        let salt:String = CommonUtils.getSalt() as String
        let sign = CommonUtils.getSign(salt)
        ...
        { task, responseObject in if let responseObject = responseObject
            {
                print("Load More Latest Channel Responce Data : \(responseObject)")
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        //4.Header Name
        self.lblheadername?.text = CommonMessage.LatestChannel()
